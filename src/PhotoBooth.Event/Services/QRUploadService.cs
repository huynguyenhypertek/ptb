...
public static class QRUploadService
{
    ...
    /// </returns>
    public static async Task<(byte[]? QrBytes, string StatusText, bool Success)> UploadAndGenerateQRBytesAsync(
        string? imagePath,
        ...
        CancellationToken ct = default)
    {
        ...
        using var qrData = qrGenerator.CreateQrCode(downloadUrl, QRCodeGenerator.ECCLevel.M);
        using var qrCode = new PngByteQRCode(qrData);
        var qrBytes = qrCode.GetGraphic(20, foregroundColor, backgroundColor);

        Console.WriteLine("[QR] Generated successfully");
        return (qrBytes, "📱 Quét mã QR để tải ảnh", true);
    }
    ...
    public static async Task<(Bitmap? QrBitmap, string StatusText, bool Success)> UploadAndGenerateQRAsync(
        ...
        CancellationToken ct = default)
    {
        var (qrBytes, statusText, success) = await UploadAndGenerateQRBytesAsync(
            imagePath, foregroundColor, backgroundColor, ct);
